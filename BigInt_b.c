...
uint32_t BigInt_divide_by_n(void* a, uint32_t divisor, void* b, BigIntType type) {
    ...
    switch(type) {
        case uint128:
            remainder = BigInt128_divide_by_n((uint128_t*)a, divisor, 0, (uint128_t*)b);
            break;
        case uint256:
            remainder = BigInt256_divide_by_n((uint256_t*)a, divisor, 0, (uint256_t*)b);
            break;
        case uint512:
            remainder = BigInt512_divide_by_n((uint512_t*)a, divisor, 0, (uint512_t*)b);
            break;
        case uint1024:
            remainder = BigInt1024_divide_by_n((uint1024_t*)a, divisor, 0, (uint1024_t*)b);
            break;
        case uint2048:
            remainder = BigInt2048_divide_by_n((uint2048_t*)a, divisor, 0, (uint2048_t*)b);
            break;
        default:
